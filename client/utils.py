...
class TwoPoints(BaseModel): # type: ignore
	x1: int
	y1: int
	x2: int
	y2: int

class RectangularShape(BaseModel): # type: ignore
	top: int
	left: int
	width: int
	height: int
	...
	def capture_screen(screen_area: Union[RectangularShape, TwoPoints]):
		with mss.mss() as sct:
			...
			if isinstance(screen_area, RectangularShape):
				...
				monitor = ScreenCapturerMSS.calc_rectangular_shape(
					screen_area
...
if __name__ == "__main__":
	NetworkHandler.post_image(
		"http://localhost:54321/",
		ScreenCapturerMSS.capture_screen(TwoPoints(x1=0, y1=0, x2=100, y2=1000)),
	)
